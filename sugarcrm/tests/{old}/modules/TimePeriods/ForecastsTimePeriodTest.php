...
class ForecastsTimePeriodTest extends TestCase
{
    ...
    public function testRebuildForecastingTimePeriods(
        ...
        $expectedDate = $timeDate->getNow()->setDate(
            $timeDate->fromDbDate($expectedSeed->start_date)->modify(
                $dateModifier
            )->format('Y'),
            $expectedMonth,
            $expectedDay
        ...
        if ($isUpgrade && $direction === 'forward') {
            $start_date = $db->getOne(
                "SELECT max(start_date) FROM timeperiods WHERE type = '{$parentType}' AND deleted = 0"
            );
        ...
        $this->assertEquals(
            ...
            $tp->start_date,
            "Failed creating {$expectedParents} new {$direction} timeperiods"
        );
        ...
        if ($isUpgrade && $direction === 'forward') {
            ...
            $start_date = $db->getOne(
                "SELECT max(start_date) FROM timeperiods WHERE type = '{$leafType}' AND deleted = 0"
            );
            $expectedDate = $timeDate->fromDbDate(substr($start_date, 0, 10));
            $this->assertEquals(
                $expectedDate->asDbDate(false),
                $tp->start_date,
                "Failed creating {$expectedLeaves} leaf timeperiods"
            );
        }
    ...
    */
    ...
    public function testCurrentTimePeriodAcrossTimeZones()
    {
        ...
        $this->assertNotEquals(
            $timeZoneACurrentTimePeriod->id,
            $timeZoneBCurrentTimePeriod->id,
            "time periods were equal, users were in same time zone"
        );

        //check that today for user a matches the timeperiod end date
        $this->assertEquals(
            $timeZoneANow->asDbDate(false),
            $timeZoneACurrentTimePeriod->end_date,
            "User in Time Zone A current date should have matched the timeperiod end date, but it didn't"
        );

        //check that today for user b matches the timeperiod start date
        $this->assertEquals(
            $timeZoneBNow->asDbDate(false),
            $timeZoneBCurrentTimePeriod->start_date,
            "User in Time Zone B current date should have matched the timeperiod start date, but it didn't"
        );
